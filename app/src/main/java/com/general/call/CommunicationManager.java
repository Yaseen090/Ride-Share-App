...
//        }
        ...
        switch (MEDIA_TYPE) {
            ...
            case TWILIO:
                ...
                break;
            case LOCAL:
                new Handler().postDelayed(() -> LocalHandler.getInstance().initiateService(mName, mImage), 1500);
                break;
            case DEFAULT:
    ...
    private void openChat(Context mContext, MediaDataProvider dataProvider) {
        if (dataProvider.media == MEDIA.DEFAULT && !Utils.checkText(dataProvider.iTripId)) {
            ...
            if (dataProvider.isBid) {
                ...
                bnChat.putString("iToMemberId", dataProvider.callId);
                bnChat.putString("iToMemberType", Utils.CALLTODRIVER);
            } else {
